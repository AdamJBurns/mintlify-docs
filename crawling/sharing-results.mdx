...
## Crawler Blocking Warning

If your recent crawls consistently return only a single broken link, you'll see a warning:

```
⚠ Crawler may be blocked — 3/5 recent crawls failed to access your site
```

This appears when **3 or more** of your last 5 crawls each discover only 1 link that's broken—indicating something is blocking our crawler at the entry point.

### Common Causes

| Cause | Solution |
| ----- | -------- |
| **WAF** (Cloudflare, Sucuri, AWS) | Whitelist User-Agent: `LinkHealthBot/1.0` |
| **Bot protection** | Disable bot fight mode or add exception |
| **robots.txt** | Allow our crawler in your robots.txt |
| **Rate limiting** | Whitelist our crawler IPs |

### Quick Fix

Check your `robots.txt` at `https://yourdomain.com/robots.txt` and ensure crawlers aren't blocked:

```
User-agent: LinkHealthBot
Allow: /
```

<Tip>
  After whitelisting, run a new crawl. A successful crawl will discover multiple pages instead of just one.
</Tip>
...
> To find navigation and other pages in this documentation, fetch the llms.txt file at: https://docs.seocrawler.app/llms.txt