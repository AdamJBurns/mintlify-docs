...
```bash
curl -X POST \
  "https://api.linkhealthmonitor.com/v1/crawl/start" \
...
<Tabs>
  <Tab title="JavaScript">
    ...
    async function startCrawl(domainId, options = {}) {
      const response = await fetch('https://api.linkhealthmonitor.com/v1/crawl/start', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${process.env.LHM_API_KEY}`,
  ...
  <Tab title="Python">
    ...
    def start_crawl(domain_id, options=None):
        ...
        response = requests.post(
            'https://api.linkhealthmonitor.com/v1/crawl/start',
            headers={
                'Authorization': f'Bearer {os.environ["LHM_API_KEY"]}',
                'Content-Type': 'application/json'
            },
            json={
                'domainId': domain_id,
  ...
  <Tab title="PHP">
    ...
    function startCrawl($domainId, $options = []) {
        ...
        curl_setopt($ch, CURLOPT_URL, 'https://api.linkhealthmonitor.com/v1/crawl/start');
        curl_setopt($ch, CURLOPT_POST, true);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_HTTPHEADER, [
            'Authorization: Bearer ' . getenv('LHM_API_KEY'),
            'Content-Type: application/json'
        ]);
