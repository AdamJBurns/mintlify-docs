...
Webhooks allow you to receive real-time HTTP notifications when events occur in SEO Crawler. Instead of polling the API, webhooks push data to your server immediately when something happens.
...
```bash
curl -X POST \
  "https://api.seocrawler.app/v1/webhooks" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "url": "https://your-server.com/webhooks/seocrawler",
...
```json
{
  ...
  "data": {
    ...
    "results_url": "https://api.seocrawler.app/v1/crawl/456e4567-e89b-12d3-a456-426614174100/results"
  }
}
```
...
<Tabs>
  <Tab title="Node.js">
    ...
    // Express middleware
    app.post('/webhooks/seocrawler', express.raw({type: 'application/json'}), (req, res) => {
      const signature = req.headers['x-seo-signature'];
  ...
  <Tab title="Python">
    ...
    @app.route('/webhooks/seocrawler', methods=['POST'])
    def handle_webhook():
        signature = request.headers.get('X-SEO-Signature')
  ...
  <Tab title="PHP">
    ...
    $signature = $_SERVER['HTTP_X_SEO_SIGNATURE'] ?? '';
...
app.post('/webhooks/seocrawler', async (req, res) => {
...
```bash
curl -X PATCH \
  "https://api.seocrawler.app/v1/webhooks/wh_123" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X POST \
  "https://api.seocrawler.app/v1/webhooks/wh_123/test" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "event": "crawl.completed"
...
# https://abc123.ngrok.io/webhooks/seocrawler
```
